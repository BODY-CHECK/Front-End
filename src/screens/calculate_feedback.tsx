...
const initialPostureCriteria = {
    0: { move1: [0, 40], move2: [0, 30], default: [0, 360], lie: [45, 135] },       // Tutorial 완료
    ...
    3: { move: [150, 190], stop1: [0, 200], stop2: [0, 200], lie: [45, 135] },      // pull up 완료
    ...
    9: { move: [170, 200], stop1: [100, 150], stop2: [60, 130], lie: [45, 135] },        // one leg squat (구현 안됨)
...
export const updateStateAndFeedback = (
    ...
    if (currentState === 'exercising') {
        ...
        if (Number(exerciseType) === 8 || Number(exerciseType) === 9){
            if (Number(exerciseType) === 8) { foot_threshold = 30 }
            else if (Number(exerciseType) === 9) { foot_threshold = 40 }
            setBooleans((prevBooleans) => ({
                ...prevBooleans,
                move: moveAngle <= thresholds.moveClear ? false : prevBooleans.move,
                stop1: Point1.x > Point2.x + foot_threshold ? true : prevBooleans.stop1,
...
export const updateStateAndFeedbackwithTime = (
    ...
    if (currentState === 'exercising') {
        if (Number(exerciseType) === 12) {
            if (moveAngle <= thresholds.moveEnd && moveAngle >= postureCriteria.move[0]) {
